...
class Dialog06ScaleDriver(hw_scale.Scale):
    ...
    def _parse_scale_answer(self, protocol, answer, regexp, parse):
        ...
        """
        _logger.debug(
            "[PARSE][SCALE] answer {} with the regexp {} and the parse {}".format(
                answer, regexp, parse
            )
        )
        ...
        try:
            _logger.debug(
                "[PARSE][SCALE] answer {} with regexp: {}".format(
                    answer, regexp
                )
            )
            ...
            if regexp and re.search(regexp, answer):
                _logger.debug(
                    "[PARSE][SCALE] answer {} with parse: {}".format(
                        answer, parse
                    )
                )
                data = parse(answer)
                ...
                if match:
                    ...
                    error = (
                        "Invalid scale answer, please power-cycle the scale"
                    )
    ...
    def send_record_01(self, unit_price, device):
        ...
        _logger.debug(
            "------------[POS][Record 01] Transmitting of unit price {} : {}".format(
    ...
    def send_record_03(self, unit_price, tare_value, device):
        ...
        _logger.debug(
            "------------[POS][Record 03] Transmitting of unit price {} and tare value {} : {}".format(
    ...
    def send_record_04(self, unit_price, text, device):
        ...
        _logger.debug(
            "------------[POS][Record 04] Transmitting of unit price {} and text {} : {}".format(
    ...
    def send_record_08(self, device):
        ...
        record_08 = "{}{}{}".format(
            self.protocol.eot_stx, self.protocol.record_08, self.protocol.etx
        ).encode("utf-8")
        _logger.debug(
            "------------[POS][Record 08] Status request after receiving <NAK> : {}".format(
                record_08
            )
        )
    ...
    def send_record_10(self, cs, kw, device):
        ...
        _logger.debug(
            "------------[POS][Record 10] Transmitting of checksums (cs : {}, kw : {}) : {}".format(
                cs, kw, record_10
            )
        )
    ...
    def send_eot_enq(self, device):
        ...
        _logger.debug(
            "------------[POS][EOT ENQ] request for weight : {}".format(
                eot_enq
            )
        )
    ...
    def send_eot(self, device):
        ...
        _logger.debug(
            "------------[POS][EOT] Resetting of scale interface : {}".format(
                eot
            )
        )
        device.write(eot)
    ...
    def _get_raw_response(self, connection):
        ...
        _logger.debug(
            "------------[SCALE][RAW] raw record : {}".format(result)
        )
        self.send_eot(
            connection
        )  # every response of the scale HAS to be answered by the POS with EOT
        return result
    ...
    def read_record_09(self, device):
        ...
        if record_no == "09":
            ...
            _logger.debug(
                "------------[SCALE][Record 09] status code : {}".format(
                    status_code
                )
            )
            return status_code
    ...
    def log_status(self, status):
        if status == "00":
            ...
            _logger.debug(
                "[SCALE][Record 09] status information: general error"
            )
        elif status == "02":
            _logger.debug(
                "[SCALE][Record 09] status information: parity status or buffer overflow"
            )
        elif status == "10":
            _logger.debug(
                "[SCALE][Record 09] status information: invalid record no."
            )
        elif status == "11":
            _logger.debug(
                "[SCALE][Record 09] status information: invalid unit price"
            )
        elif status == "12":
            _logger.debug(
                "[SCALE][Record 09] status information: invalid tare value"
            )
        elif status == "13":
            _logger.debug(
                "[SCALE][Record 09] status information: invalid text"
            )
        elif status == "20":
            _logger.debug(
                "[SCALE][Record 09] status information: scale is still in motion"
            )
        elif status == "21":
            _logger.debug(
                "[SCALE][Record 09] status information: scale was not in motion since last operation"
            )
        elif status == "22":
            _logger.debug(
                "[SCALE][Record 09] status information: measurement is not yet finished"
            )
        elif status == "30":
            _logger.debug(
                "[SCALE][Record 09] status information: weight is less than minimum weight"
            )
        elif status == "31":
            _logger.debug(
                "[SCALE][Record 09] status information: scale is less than 0"
            )
        elif status == "32":
            _logger.debug(
                "[SCALE][Record 09] status information: scale is overloaded"
            )
        self.error = status
    ...
    def set_device(self):
        if not self.device:
            try:
                _logger.debug(
                    "[DEVICE] Probing %s with protocol %s",
                    self.path_to_scale,
                    protocol,
                )
                ...
                if connected:
                    _logger.info(
                        "Probing %s: answer looks ok for protocol %s",
                        self.path_to_scale,
                        protocol.name,
                    )
                    self.set_status(
                        "connected",
                        "Connected to %s with %s protocol"
                        % (self.device, protocol.name),
                    )
                    self.device.timeout = protocol.timeout
                    self.device.writeTimeout = protocol.writeTimeout
                else:
                    _logger.info(
                        "Probing %s: no valid answer to protocol %s",
                        self.path_to_scale,
                        protocol.name,
                    )
                    self.set_status(
                        "disconnected", "No supported USB scale found"
                    )
                    self.protocol = None
                    self.device = None
    ...
    def calculate_checksums(self, scale_answer, device):
        ...
        if not device:
            _logger.debug(
                "[CALCULATE CHECKSUMS] Device information not transmitted to initialization function"
            )
        else:
            try:
                ...
                d0, z = data
                _logger.debug(
                    "[CALCULATE CHECKSUMS] Record 11 returns d0 {} and Z {}".format(
                        d0, z
                    )
                )
                _logger.debug(
                    "[CALCULATE CHECKSUMS] Checksum and correction value generation with the number {}".format(
                        z
                    )
                )
                ...
                kw_hex_ascii_encoded = "{:04X}".format(
                    int(kw_bin_encoded, 2), "X"
                )

                _logger.debug(
                    "[CALCULATE CHECKSUMS] Checksum chosen before encoding: {}".format(
                        cs
                    )
                )
                _logger.debug(
                    "[CALCULATE CHECKSUMS] Checksum after encoding: {}".format(
                        cs_hex_ascii_encoded
                    )
                )
                _logger.debug(
                    "[CALCULATE CHECKSUMS] Correction value after encoding: {}".format(
                        kw_hex_ascii_encoded
                    )
                )
            except Exception as e:
                self.set_status(
                    "error",
                    "During generation of checksum and correction value with Exception {}".format(
                        e
                    ),
                )
                self.device = None
            ...
            if answer_checksum == NAK:
                ...
                if answer_status == NAK:
                    ...
                    if record_no == "11":
                        ...
                        d0, z = data
                        _logger.debug(
                            "[SEND CHECKSUM] Record 11 returns D0 {} and Z {}".format(
                                d0, z
                            )
                        )
                        if d0 == "1":
                            _logger.debug(
                                "[SEND CHECKSUM] valid checksum: D0 = {}".format(
                                    d0
                                )
                            )
                            connected = True
                            ...
                            _logger.debug(
                                "[SEND CHECKSUM] invalid checksum: D0 = {}".format(
                                    d0
                                )
                            )
            else:
    ...
    def request_weighing_operation(self, price, tare=None, text=None):
        try:
            _logger.debug(
                "[WEIGHING] POS transmits one of the Records 01, 03, 04 or 05"
            )
            with self.scalelock:
            ...
            _logger.debug(
                "[WEIGHING] Could not weigh on scale {} with protocol {}: {}".format(
                    self.path_to_scale, self.protocol.name, e
                )
            )
            self.set_status(
    ...
    def _handle_weighing_answer(self, answer):
        ...
        # if no errors, the scale answers with <ACK>
            ...
            if weighing_result == NAK:
                ...
                if record_no == "02":
                    _logger.debug(
                        "[WEIGHING][Record 02] Weight data received."
                    )
                    ...
                    _logger.debug(
                        "[WEIGHING][DATA] uom, weight, priceKg, price updated."
                    )
                    _logger.debug(
                        "[WEIGHING][DATA] weight = {}".format(self.weight)
                    )
                    _logger.debug(
                        "[WEIGHING][DATA] weight = {}".format(
                            self.get_weight()
                        )
                    )
                else:
                    _logger.debug(
                        "[WEIGHING][RESULT] Weight data is not received after an ACK answer."
                    )
                    _logger.debug(
                        "[WEIGHING][RESULT] Frame received is {} with status {}".format(
                            weighing_result, str(record_no)
                        )
                    )
                    error = self.request_status_information(self.device)
        else:
            ...
            _logger.debug(
                "[WEIGHING] The frame received is neither ACK nor NAK."
            )
            record_no, error = self._parse_scale_answer(
                self.protocol,
                answer,
            ...
            if record_no == "11":
                ...
                _logger.debug(
                    "[WEIGHING] Record 11 is not received after a NAK answer."
                )
                _logger.debug(
                    "[WEIGHING] Frame received is {} with status {}".format(
                        answer, str(record_no)
                    )
                )
        return connected, error
