...
def _parse_record_no(data):
    ...
    stx = data.index("\x02")  # STX (Start of Text) ASCII control character
...
def _parse_weighing_result(data):
    ...
    record_no, uom, scale_weight, unit_price, total_price = (
        None,
        None,
        None,
        None,
    ...
    data = data.decode("utf-8")
    if record_no == "02":
        esc_indices = [i for i, x in enumerate(data) if x == "\x1b"]
    ...
    _logger.debug(
        "[PARSE][WEIGHING] uom : {}, weight : {}, price_kg : {} and price : {}".format(
...
Dialog06Protocol = ScaleProtocol(
    ...
    eot_stx="\x04\x02",  # EOT and Start of Text ASCII control characters
    etx="\x03",  # End of Text ASCII control character
    esc="\x1b",  # Escape ASCII control character
    eot_enq="\x04\x05",  # EOT and Enquiry ASCII control characters
    eot="\x04",  # End of Transmission ASCII control character
    ack_regexp=b"",
